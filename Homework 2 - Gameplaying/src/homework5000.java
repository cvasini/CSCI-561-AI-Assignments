...
public class homework5000 {
	...
	public static void main(String [] args) throws IOException {
		int nthInput = 1;
		for(nthInput = 1; nthInput<=4999; nthInput++) {
			if(nthInput != 449 && nthInput != 821 && nthInput != 1371 && nthInput != 2767) {
				List<String> lines = Files.readAllLines(Paths.get("Test cases/testcases-5000/INPUT", Integer.toString(nthInput)+".in"));
			...
			if(algo.equals("MINIMAX")){
				answerMove = new homework5000().minimaxDecision(origBoardState, depth, true);
			}
			else if(algo.equals("ALPHABETA")){
				answerMove = new homework5000().alphabetaDecision(origBoardState, depth, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, true);
			}
			...
			//create output file
			PrintWriter writer = new PrintWriter("Test cases/Output for 5000 cases/"+Integer.toString(nthInput)+".out", "UTF-8");
			...
			writer.print("\r\n");
			writer.close();
			Path file1 = Paths.get("Test cases/Output for 5000 cases/", Integer.toString(nthInput)+".out");
			Path file2 = Paths.get("Test cases/testcases-5000/OUTPUT/", Integer.toString(nthInput)+".out");
	...
	public LinkedHashMap<String, String[][]> generateMoves(String[][] currBoardState, String currentPlayer){
		...
		raidMoves = new homework5000().raidPossible(currBoardState, currentPlayer);
		for(int i= 0; i<sizeOfBoard; i++){
			for(int j = 0; j< sizeOfBoard;j++){
				if(currBoardState[i][j].equals(".")) {
					String[][] newBoardState = new homework5000().copy2DStringArray(currBoardState);
	...
	public LinkedHashMap<String, Object> minimaxDecision(String[][] boardState, int d, boolean maximizingPlayer){
		if(d == 0 || !new homework5000().isTerminalState(boardState)){
			LinkedHashMap<String, Object> best = new LinkedHashMap<String, Object>();
			best.put("bestValue", new homework5000().calcGameScore(boardState));
			return best;
		}
		if(maximizingPlayer){
			LinkedHashMap<String, Object> best = new LinkedHashMap<String, Object>();
			best.put("bestValue", Double.NEGATIVE_INFINITY);
			LinkedHashMap<String, String[][]> moves = new homework5000().generateMoves(boardState, maxPlayer);
			Iterator<Entry<String, String[][]>> itMoves = moves.entrySet().iterator();
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework5000().minimaxDecision((String[][]) move.getValue(), d - 1, false);
			...
			LinkedHashMap<String, String[][]> moves = new homework5000().generateMoves(boardState, minPlayer);
			Iterator<Entry<String, String[][]>> itMoves = moves.entrySet().iterator();
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework5000().minimaxDecision((String[][])move.getValue(), d - 1, true);
	...
	public LinkedHashMap<String, Object> alphabetaDecision(String[][] boardState, int d, Double alpha, Double beta, boolean maximizingPlayer){
		...
		if(maximizingPlayer){
			...
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework5000().alphabetaDecision((String[][]) move.getValue(), d - 1, alpha, beta, false);
				if((Double)best.get("bestValue") < (Double)score.get("bestValue")){
					best.put("bestValue", score.get("bestValue"));
					best.put("bestMoveState", move.getValue());
					best.put("bestMove", move.getKey());
			...
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework5000().alphabetaDecision((String[][])move.getValue(), d - 1,alpha, beta, true);
				if((Double)score.get("bestValue")<(Double) best.get("bestValue")){
					best.put("bestValue", score.get("bestValue"));
					best.put("bestMoveState", move.getValue());
					best.put("bestMove", move.getKey());
