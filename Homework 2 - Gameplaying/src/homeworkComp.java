...
public class homeworkComp {
	...
	public static void main(String [] args) throws IOException {
		...
		if(algo.equals("MINIMAX")){
			...
			answerMove = new homeworkComp().minimaxDecision(origBoardState, depth, true);
		}
		else if(algo.equals("ALPHABETA")){
			depth = Integer.parseInt(lines.get(3));
			answerMove = new homeworkComp().alphabetaDecision(origBoardState, depth, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, true);
		} else if(algo.equals("COMPETITION")) {
			cpuTime = Float.valueOf(lines.get(3));
			if(sizeOfBoard >= 11) {
				depth = 3;
			}
			if(sizeOfBoard >= 8 && sizeOfBoard <= 10) {
				depth = 4;
			}
			if(sizeOfBoard == 6) {
				depth = 5;
	...
	public LinkedHashMap<String, String[][]> generateMoves(String[][] currBoardState, String currentPlayer){
		...
		raidMoves = new homeworkComp().raidPossible(currBoardState, currentPlayer);
		for(int i= 0; i<sizeOfBoard; i++){
			for(int j = 0; j< sizeOfBoard;j++){
				if(currBoardState[i][j].equals(".")) {
					String[][] newBoardState = new homeworkComp().copy2DStringArray(currBoardState);
				...
				if(generatedBoardState[row][col].equals(".")) {
					...
					if(!currPlayerPositions.isEmpty()){
						...
						if(!raidPositions.isEmpty()){
							String[][] newBoardState = new homeworkComp().copy2DStringArray(generatedBoardState);
	...
	public LinkedHashMap<String, Object> minimaxDecision(String[][] boardState, int d, boolean maximizingPlayer){
		if(d == 0 || !new homeworkComp().isTerminalState(boardState)){
			LinkedHashMap<String, Object> best = new LinkedHashMap<String, Object>();
			best.put("bestValue", new homeworkComp().calcGameScore(boardState));
			return best;
		}
		if(maximizingPlayer){
			LinkedHashMap<String, Object> best = new LinkedHashMap<String, Object>();
			best.put("bestValue", Double.NEGATIVE_INFINITY);
			LinkedHashMap<String, String[][]> moves = new homeworkComp().generateMoves(boardState, maxPlayer);
			Iterator<Entry<String, String[][]>> itMoves = moves.entrySet().iterator();
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homeworkComp().minimaxDecision((String[][]) move.getValue(), d - 1, false);
			...
			LinkedHashMap<String, String[][]> moves = new homeworkComp().generateMoves(boardState, minPlayer);
			Iterator<Entry<String, String[][]>> itMoves = moves.entrySet().iterator();
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homeworkComp().minimaxDecision((String[][])move.getValue(), d - 1, true);
	...
	public LinkedHashMap<String, Object> alphabetaDecision(String[][] boardState, int d, Double alpha, Double beta, boolean maximizingPlayer){
		...
		if(maximizingPlayer){
			...
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homeworkComp().alphabetaDecision((String[][]) move.getValue(), d - 1, alpha, beta, false);
				if((Double)best.get("bestValue") < (Double)score.get("bestValue")){
					best.put("bestValue", score.get("bestValue"));
					best.put("bestMoveState", move.getValue());
					best.put("bestMove", move.getKey());
			...
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homeworkComp().alphabetaDecision((String[][])move.getValue(), d - 1,alpha, beta, true);
				if((Double)score.get("bestValue")<(Double) best.get("bestValue")){
					best.put("bestValue", score.get("bestValue"));
					best.put("bestMoveState", move.getValue());
					best.put("bestMove", move.getKey());
