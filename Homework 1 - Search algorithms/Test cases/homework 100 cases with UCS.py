...
for nthInput in range(1000, 1100):
    f = open("UCS/Input/CROSSFIRE"+str(nthInput)+".txt")
    lines = f.readlines()
    algo = "UCS"
    ...
    states = set()
    sundayLines = {}
    with open("UCS/Input/CROSSFIRE"+str(nthInput)+".txt") as f:
    ...
    visited = []
    #create output file
    if os.path.isfile("UCSOutput/CROSSFIRE"+str(nthInput)+".txt"):
        wf = open("UCSOutput/CROSSFIRE"+str(nthInput)+".txt", 'w')
        wf.truncate()
    else:
        wf = open("UCSOutput/CROSSFIRE"+str(nthInput)+".txt", 'w')
    ...
    if result:
            ...
            if algo == "BFS" or algo == "DFS":
                ...
                wf.close()
    w1 = open("UCSOutput/CROSSFIRE"+str(nthInput)+".txt")
    aa = []
    bb = []
    for n in w1:
        aa.append(n)
    w2 = open("UCS/Output/CROSSFIRE"+str(nthInput)+".txt")
    for n in w2:
        bb.append(n)
