...
import os.path
import filecmp
for nthInput in range(205):
    f = open("Test cases - 205/Input/input"+str(nthInput)+".txt")
    lines = f.readlines()
    algo = lines[0].rstrip('\n')
    ...
    states = set()
    sundayLines = {}
    with open("Test cases - 205/Input/input"+str(nthInput)+".txt") as f:
    ...
    visited = []
    #create output file
    if os.path.isfile("Output for 205 cases/output"+str(nthInput)+".txt"):
        wf = open("Output for 205 cases/output"+str(nthInput)+".txt", 'w')
        wf.truncate()
    else:
        wf = open("Output for 205 cases/output"+str(nthInput)+".txt", 'w')
    ...
    if result:
            ...
            if algo == "BFS" or algo == "DFS":
                ...
                wf.close()
    if not filecmp.cmp("Output for 205 cases/output"+str(nthInput)+".txt", "Test cases - 205/Output/output"+str(nthInput)+".txt"):
