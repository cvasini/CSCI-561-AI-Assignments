...
for nthInput in range(1000, 1100):
    f = open("AStar/Input/CROSSFIRE"+str(nthInput)+".txt")
    lines = f.readlines()
    algo = "A*"
    ...
    states = set()
    sundayLines = {}
    with open("AStar/Input/CROSSFIRE"+str(nthInput)+".txt") as f:
    ...
    visited = []
    #create output file
    if os.path.isfile("AStarOutput/CROSSFIRE"+str(nthInput)+".txt"):
        wf = open("AStarOutput/CROSSFIRE"+str(nthInput)+".txt", 'w')
        wf.truncate()
    else:
        wf = open("AStarOutput/CROSSFIRE"+str(nthInput)+".txt", 'w')
    ...
    if result:
            ...
            if algo == "BFS" or algo == "DFS":
                ...
                wf.close()
    w1 = open("AStarOutput/CROSSFIRE"+str(nthInput)+".txt")
    aa = []
    bb = []
    for n in w1:
        aa.append(n)
    w2 = open("AStar/Output/CROSSFIRE"+str(nthInput)+".txt")
    for n in w2:
        bb.append(n)
