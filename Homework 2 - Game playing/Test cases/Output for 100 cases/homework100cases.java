...
public class homework100cases {
	...
	public static void main(String [] args) throws IOException {
		int nthInput = 0;
		for(nthInput = 0; nthInput<=99; nthInput++) {
			List<String> lines = Files.readAllLines(Paths.get("testcases/"+Integer.toString(nthInput)+".in"));
			...
			if(algo.equals("MINIMAX")){
				answerMove = new homework100cases().minimaxDecision(origBoardState, depth, true);
			}
			else if(algo.equals("ALPHABETA")){
				answerMove = new homework100cases().alphabetaDecision(origBoardState, depth, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, true);
			}
			String [] listMove = ((String) answerMove.get("bestMove")).split(" ");
			if(((String) answerMove.get("bestMove")).contains("to") == false) {
				actualMove = lookUpAlphabet.get(Integer.parseInt(listMove[1])) + Integer.toString(Integer.parseInt(listMove[0]) + 1) + " Stake";
			}
			else{
				actualMove = lookUpAlphabet.get(Integer.parseInt(listMove[4])) + Integer.toString(Integer.parseInt(listMove[3]) + 1) + " Raid";
			}
			//create output file
			PrintWriter writer = new PrintWriter(Integer.toString(nthInput)+".out", "UTF-8");
			writer.println(actualMove);
			...
			writer.close();
			Path file1 = Paths.get(Integer.toString(nthInput)+".out");
			Path file2 = Paths.get("testcases/", Integer.toString(nthInput)+".out");
			byte[] f1 = Files.readAllBytes(file1);
			byte[] f2 = Files.readAllBytes(file2);
			Arrays.equals(f1, f2);
			if(!Arrays.equals(f1, f2)) {
				System.out.println("Testcase"+nthInput+"failed");
				System.out.println(answerMove);
			}
			if(nthInput == 99){
	...
	public LinkedHashMap<String, String[][]> generateMoves(String[][] currBoardState, String currentPlayer){
		...
		raidMoves = new homework100cases().raidPossible(currBoardState, currentPlayer);
		for(int i= 0; i<sizeOfBoard; i++){
			for(int j = 0; j< sizeOfBoard;j++){
				if(currBoardState[i][j].equals(".")) {
					String[][] newBoardState = new homework100cases().copy2DStringArray(currBoardState);
	...
	public LinkedHashMap<String, String[][]> raidPossible(String[][] generatedBoardState, String currentPlayer){
		List<LinkedHashMap<String, Integer>> emptyPositions = new ArrayList<LinkedHashMap<String, Integer>>();
		...
		for(int row= 0; row<sizeOfBoard; row++){
			for(int col = 0; col< sizeOfBoard;col++){
				if(generatedBoardState[row][col].equals(currentPlayer)) {
					emptyPositions = adjacentCheck(row, col, ".", generatedBoardState);
					for(int i=0;i<emptyPositions.size();i++){
						raidPositions = adjacentCheck(emptyPositions.get(i).get("left"), emptyPositions.get(i).get("right"), oppoPlayer, generatedBoardState);
						if(!raidPositions.isEmpty()){
							String[][] newBoardState = new homework100cases().copy2DStringArray(generatedBoardState);
							for(int j=0;j<raidPositions.size();j++){
								newBoardState[raidPositions.get(j).get("left")][raidPositions.get(j).get("right")] = currentPlayer;
								newBoardState[emptyPositions.get(i).get("left")][emptyPositions.get(i).get("right")] = currentPlayer;
							}
							raidMoves.put(Integer.toString(row)+" "+ Integer.toString(col)+" to "+Integer.toString(emptyPositions.get(i).get("left"))+" "+emptyPositions.get(i).get("right"), newBoardState);
						}
					}
				}
			}
	...
	public LinkedHashMap<String, Object> minimaxDecision(String[][] boardState, int d, boolean maximizingPlayer){
		if(d == 0 || !new homework100cases().isTerminalState(boardState)){
			LinkedHashMap<String, Object> best = new LinkedHashMap<String, Object>();
			best.put("bestValue", new homework100cases().calcGameScore(boardState));
			return best;
		}
		if(maximizingPlayer){
			LinkedHashMap<String, Object> best = new LinkedHashMap<String, Object>();
			best.put("bestValue", Double.NEGATIVE_INFINITY);
			LinkedHashMap<String, String[][]> moves = new homework100cases().generateMoves(boardState, maxPlayer);
			Iterator<Entry<String, String[][]>> itMoves = moves.entrySet().iterator();
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework100cases().minimaxDecision((String[][]) move.getValue(), d - 1, false);
			...
			LinkedHashMap<String, String[][]> moves = new homework100cases().generateMoves(boardState, minPlayer);
			Iterator<Entry<String, String[][]>> itMoves = moves.entrySet().iterator();
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework100cases().minimaxDecision((String[][])move.getValue(), d - 1, true);
	...
	public LinkedHashMap<String, Object> alphabetaDecision(String[][] boardState, int d, Double alpha, Double beta, boolean maximizingPlayer){
		...
		if(maximizingPlayer){
			...
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework100cases().alphabetaDecision((String[][]) move.getValue(), d - 1, alpha, beta, false);
				if((Double)best.get("bestValue") < (Double)score.get("bestValue")){
					best.put("bestValue", score.get("bestValue"));
					best.put("bestMoveState", move.getValue());
					best.put("bestMove", move.getKey());
			...
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework100cases().alphabetaDecision((String[][])move.getValue(), d - 1,alpha, beta, true);
				if((Double)score.get("bestValue")<(Double) best.get("bestValue")){
					best.put("bestValue", score.get("bestValue"));
					best.put("bestMoveState", move.getValue());
					best.put("bestMove", move.getKey());
