...
public class homework {
	...
	public static void main(String [] args) throws IOException {
		...
		if(algo.equals("MINIMAX")){
			answerMove = new homework().minimaxDecision(origBoardState, depth, true);
		}
		else if(algo.equals("ALPHABETA")){
			answerMove = new homework().alphabetaDecision(origBoardState, depth, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, true);
		}
	...
	public LinkedHashMap<String, String[][]> generateMoves(String[][] currBoardState, String currentPlayer){
		...
		raidMoves = new homework().raidPossible(currBoardState, currentPlayer);
		for(int i= 0; i<sizeOfBoard; i++){
			for(int j = 0; j< sizeOfBoard;j++){
				if(currBoardState[i][j].equals(".")) {
					String[][] newBoardState = new homework().copy2DStringArray(currBoardState);
	...
	public LinkedHashMap<String, Object> minimaxDecision(String[][] boardState, int d, boolean maximizingPlayer){
		if(d == 0 || !new homework().isTerminalState(boardState)){
			LinkedHashMap<String, Object> best = new LinkedHashMap<String, Object>();
			best.put("bestValue", new homework().calcGameScore(boardState));
			return best;
		}
		if(maximizingPlayer){
			LinkedHashMap<String, Object> best = new LinkedHashMap<String, Object>();
			best.put("bestValue", Double.NEGATIVE_INFINITY);
			LinkedHashMap<String, String[][]> moves = new homework().generateMoves(boardState, maxPlayer);
			Iterator<Entry<String, String[][]>> itMoves = moves.entrySet().iterator();
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework().minimaxDecision((String[][]) move.getValue(), d - 1, false);
			...
			LinkedHashMap<String, String[][]> moves = new homework().generateMoves(boardState, minPlayer);
			Iterator<Entry<String, String[][]>> itMoves = moves.entrySet().iterator();
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework().minimaxDecision((String[][])move.getValue(), d - 1, true);
	...
	public LinkedHashMap<String, Object> alphabetaDecision(String[][] boardState, int d, Double alpha, Double beta, boolean maximizingPlayer){
		...
		if(maximizingPlayer){
			...
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework().alphabetaDecision((String[][]) move.getValue(), d - 1, alpha, beta, false);
				if((Double)best.get("bestValue") < (Double)score.get("bestValue")){
					best.put("bestValue", score.get("bestValue"));
					best.put("bestMoveState", move.getValue());
					best.put("bestMove", move.getKey());
			...
			while(itMoves.hasNext()) {
				Entry<String, String[][]> move = itMoves.next();
				LinkedHashMap<String, Object> score = new homework().alphabetaDecision((String[][])move.getValue(), d - 1,alpha, beta, true);
				if((Double)score.get("bestValue")<(Double) best.get("bestValue")){
					best.put("bestValue", score.get("bestValue"));
					best.put("bestMoveState", move.getValue());
					best.put("bestMove", move.getKey());
